...
async function buildVectorStore(pdfPath) {
  ...
  for (let i = 0; i < vectors.length; i += UPSERT_BATCH_SIZE) {
    ...
    console.log("Sample vector id:", batch[0]?.id);
    console.log("Sample values length:", batch[0]?.values?.length);
    console.log("Sample values type:", typeof batch[0]?.values);

    await pineconeIndex.upsert({ records: batch });  // ✅ v7 format
  }
